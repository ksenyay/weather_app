...
class UIManager {
  ...
  static updateFeelsLike(feelsLike) {
    document.querySelector(".feels-like").textContent = `Feels like ${Math.round(feelsLike)} °C`;
  }
  ...
  static updateHourlyWeather(hourlyData) {
    ...
    for (let i = 0; i <= 23; i += 3) {
      ...
      if (hour) {
        document.querySelector(`#hourly-${index + 1} div p`).textContent = `${Math.round(hour.temp_c)} °C`;
        document.querySelector(`#hourly-${index + 1} div img`).src = hour.condition.icon;
        index += 1;
      }
    }
    document.querySelector(`#hourly-9 div p`).textContent = `${Math.round(hourlyData.hour[23].temp_c)} °C`;
...
class EventHandler {
  ...
  async fetchAndDisplayWeather(location) {
    try {
      ...
      const currentWeatherData = await this.apiHandler.fetchCurrentWeather(location);
...
window.onload = () => {
  ...
  document.querySelector("form").addEventListener("submit", event => eventHandler.handleFormSubmit(event));
...
class ThemeManager {
  ...
  async setBackground(value) {
    ...
    if (imageExists) {
      ...
      localStorage.setItem("backgroundValue", `${this.defaultBg[0].toUpperCase()}${this.defaultBg.slice(1)}`);
    }
  }
  openDropdown(event) {
  ...
  init() {
    this.dropdownButton.addEventListener("click", event => this.openDropdown(event));
